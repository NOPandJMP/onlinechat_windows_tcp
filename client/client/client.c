...
int main()
{
    ...
    printf("WELCOME TO EBUSI CHAT\n");

    if (WSAStartup(MAKEWORD(2, 2), &wsa) != 0)//Создание сокета 
    {
    ...
    server.sin_family = AF_INET;//IPv4 протокол
    server.sin_port = htons(7777);//Номер порта для конекта

ip_reconnect://это для реконекта , знаю ,что goto для дрявых но мне впадлу переделывать на нормальное возвращение
    printf("\nReady to Connect. Enter the IP Address of the server to connect.\n");
    scanf("%s", &ip_server);//Ввод ip серевра
...
re_pass:
    ...
    while (1)
    {
        ...
        if (ch == 8) /*id дял бэкса писал в сервере*/
        {
            printf("\b \b");
            i--;
            pass[i] = '\0';
