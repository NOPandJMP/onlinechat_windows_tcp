...
FILE* fp;
fd_set readfds;//набор дескрипторов сокетов
...
init:
    ...
    printf("WELCOME TO EBULSI CHATT\n");
    ...
    while (TRUE)
    {
        ...
        //Если словили коннект главном сокете , то это входящее соединение
        if (FD_ISSET(master_socket, &readfds))
        {
            ...
            //сообщение пользователю номер сокета
            ...
            for (tracker = 0; tracker < MAX_CLIENT; tracker++)
            {
                if (client_socket[tracker] == 0)
                {
                    ...
                    //FILE* inputf = fopen("history.txt", "w");
                    //
                    //    // Читаем информацию из файла и отправляем её клиенту.
                    //    while (!feof(inputf)) {
                    //        size_t nf = fread(buffer, sizeof(char), BUFSIZ, inputf);
        ...
        {
            sock = client_socket[tracker];
            //Если есть клиент , читаем
            if (FD_ISSET(sock, &readfds))
            {
                //получаем детали от клиента
                getpeername(sock, (struct sockaddr*)&address, (int*)&addrlen);
                //Проверяем закрытие , а так же входящие соеденение
                
                rcvd_size = recv(sock, rcvd_msg, MSG_SIZE, 0);
              /*  FILE* fout;
                fout = fopen("history.txt", "w");
                fprintf(fout, "%s", rcvd_msg);
                fclose(fout);*/
...
int show_menu()
{
    ...
    case 0:
        ...
        printf("\t\t\t********************************\n");
        printf("\t\t\t    WELCOME TO EBUSI CHAT %s\n", user);
        printf("\t\t\t********************************\n\n\n");
        break;
    case 1:
        printf("\t\t\t       \n");
...
re_pass:
    ...
    while (1)
    {
        ...
        if (ch == 8) /*Значение бэкспэйса точнее его id на клаве*/
        {
            printf("\b \b");
            i--;
            pass[i] = '\0';
